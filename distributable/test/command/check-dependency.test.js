...
Test.serial('check-dependency --help', async test => {
  let process = new CheckDependencyProcess({ '--help': true });
  test.is(await process.whenExit(), 0);
});

Test.serial('check-dependency', async test => {
...
Test.serial('check-dependency --project-path missing --report-missing', async test => {
  let process = new CheckDependencyProcess({ '--project-path': `${ResourcePath}/missing`, '--report-missing': true });
  test.is(await process.whenExit(), 1);
});

Test.serial('check-dependency --project-path missing --no-report-missing', async test => {
  let process = new CheckDependencyProcess({ '--project-path': `${ResourcePath}/missing`, '--no-report-missing': true });
  test.is(await process.whenExit(), 0);
});

Test.serial('check-dependency --project-path unused --report-unused', async test => {
  let process = new CheckDependencyProcess({ '--project-path': `${ResourcePath}/unused`, '--report-unused': true });
  test.is(await process.whenExit(), 1);
});

Test.serial('check-dependency --project-path unused --no-report-unused', async test => {
...
Test.serial('check-dependency --project-path error', async test => {
